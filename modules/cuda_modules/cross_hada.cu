...
crossHadamardKernel(const scalar_t *matrices, scalar_t *result, int p, int c, int h, int w)
{
    ...
    scalar_t *i_tile  = reinterpret_cast<scalar_t *>(shm);
    scalar_t *j_tile  = i_tile + blockDim.y * blockDim.z;
    ...
    i_tile[tile_id] = matrices[(bid * c + i) * h * w + matg_id];
    j_tile[tile_id] = matrices[(bid * c + j) * h * w + matg_id];

    result[(bid * p + pid) * h * w + matg_id] = i_tile[tile_id] * j_tile[tile_id];
}
                                         ...
                                         int             k)
{
    ...
    i_tile[tile_id] = matrices[(bid * c + indices[bid * k + i]) * h * w + matg_id];
    j_tile[tile_id] = matrices[(bid * c + indices[bid * k + j]) * h * w + matg_id];
...
torch::Tensor crossHadamard(torch::Tensor matrices)
{
    ...
    AT_DISPATCH_FLOATING_TYPES(
        matrices.scalar_type(), "cross_hadamard",
        (
            [&]
            {
                crossHadamardKernel<scalar_t>
                    <<<block, thread, 2 * thd_edge * thd_edge * sizeof(scalar_t)>>>(
                        matrices.data_ptr<scalar_t>(), output.data_ptr<scalar_t>(), p, c, h, w);
            }));
...
torch::Tensor crossHadamardMixed(torch::Tensor matrices, torch::Tensor logits, const int k)
{
    ...
    AT_DISPATCH_FLOATING_TYPES(
        matrices.scalar_type(), "cross_hadamard_mixed",
        (
            [&]
            {
                crossHadamardMixedKernel<scalar_t>
                    <<<block, thread, 2 * thd_edge * thd_edge * sizeof(scalar_t)>>>(
                        indices_ptr, matrices.data_ptr<scalar_t>(), output.data_ptr<scalar_t>(), p,
                        c, h, w, k);
            }));
